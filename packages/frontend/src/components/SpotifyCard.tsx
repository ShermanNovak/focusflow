...
export default function SpotifyCard({ showModal } : { showModal: React.Dispatch<React.SetStateAction<boolean>> }) {
  const cookies = new Cookies();

  const handleClick = async () => {
    if (cookies.get("spotify-refresh-token")) {
  ...
  return (
    <div 
      ...
      className="flex items-center rounded-md w-max cursor-pointer"
      onClick={() => handleClick()}
    >
      <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0,0,256,256" style={{fill:"#000000"}} className="ml-[25px]">
      ...
      {cookies.get("spotify-refresh-token") && <div className="text-[14px] text-white mr-[25px] ml-[14px] my-[15px]">
