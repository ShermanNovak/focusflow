...
export default function SpotifyWebPlayback({ selectedSong, handleShowModal }: { 
    ...
    <>
      <div className="flex">
          ...
          <div className="text-[14px] text-white mr-[17px] ml-[17px] my-[15px] text-ellipsis whitespace-nowrap overflow-hidden w-[200px] cursor-pointer" onClick={() => handleShowModal(true)}>
            ...
            <p className="m-0 text-ellipsis text-ellipsis overflow-hidden">{selectedSong.artists.map((artist: Spotify.Entity) => artist.name).join(", ")}</p>
          </div>
          {player && <span className="h-8 w-8 my-auto mr-[20px] cursor-pointer" onClick={() => player.togglePlay()}>
            {!is_paused 
