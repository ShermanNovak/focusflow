...
class SOTDController {
    public async getSongs(req: AuthenticatedRequest, res: Response) {
        const songs = await SongOfTheDay.find({ user: req.user_id }).sort({ createdAt: -1 });
        res.json(songs);
    }

    public async getSong(req: AuthenticatedRequest, res: Response) {
        const song = await SongOfTheDay.findById(req.params.id);
    ...
    public async createSong(req: AuthenticatedRequest, res: Response) {
        try {
            const song = await SongOfTheDay.create({...req.body, user: req.user_id});
            res.json(song);
        } catch (error: any){
    ...
    public async updateSong(req: AuthenticatedRequest, res: Response) {
        const song = await SongOfTheDay.findByIdAndUpdate(req.params.id, req.body, { returnDocument: 'after' });
        if (!song) return res.status(404).json({ error: 'No such song' });
